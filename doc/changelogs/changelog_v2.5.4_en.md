...
- **Auto-update prompt after changing the update source**: When the update repository is changed (mode or value) and saved, Kittyhack now offers to run the update immediately. The new source typically has a different code base, so this saves an extra manual step.
- **Force update button**: The *Information* tab now has a "Force update now" button that is always visible. It re-runs the update against the configured source — useful to refresh the current branch head (`git checkout -B <ref> origin/<ref>`) or to re-install the selected tag without waiting for a version bump.
